...
const sessionsReducer = (state = initialState, action) => {
  ...
  switch (action.type) {
    ...
    case 'DELETE_SESSION':
     return Object.assign({}, state, {
       ...
       activeSession : { ...state.activeSession,
         ...
         MetricsData: (action.payload.id === state.activeSession.id) ? '' : state.activeSession.MetricsData,
         CompareSolution: (action.payload.id === state.activeSession.id) ? '' : state.activeSession.CompareSolution,
       }  
    });
    ...
    case 'EPA_ACTV_SOLN_CHANGE': 
      console.log(action.payload.data.actvItem);
      let prevItem = [...state.activeSession.EPAArray];

      let newEPAarr = [];
      prevItem.map((item, index) => {
        if (item.treeView.Solutions.Solution.SolProfile['#text'] !== action.payload.data.actvItem){
            item.treeView = state.activeSession.treeView;
            item.bomView = state.activeSession.bomView;
            item.configView = state.activeSession.configView;
            item.GRInput = state.activeSession.GRInput;
            item.GRBomInput = state.activeSession.GRBomInput;
            newEPAarr.push(item);
        } else{ newEPAarr.push(item); }
      });
      ...
      return {
        ...
        activeSession : {...state.activeSession,
                          EPAArray: newEPAarr,
                          treeView : newitem.treeView,
                          bomView : newitem.bomView,
                          configView : newitem.configView,
                          GRInput : newitem.GRInput,
                          GRBomInput : newitem.GRBomInput
      }
      }

    case 'FETCH_RA_BOM_PENDING' :
      return Object.assign({}, state, {
        isLoading: true,
        LoadingMsg: 'Please wait while fetching the RA BOM data.'
    });
    
    case 'FETCH_RA_BOM_SUCCESS' :
      return {
              ...state,
              RABOM : action.payload,
              isLoading:false,
              LoadingMsg: ''
    };
    ...
    case 'FETCH_METRICS_DATA_PENDING':
      return Object.assign({}, state, {
       isLoading: true,
       LoadingMsg: 'Please wait while fetching the Metrics data.'
    });

    case 'FETCH_METRICS_DATA_SUCCESS':
       console.log(action);
       var actvSession = state.activeSession;
       actvSession.MetricsData = action.payload;
       return {
         ...state,
    ...
    case 'FETCH_COMPARE_SOLUTION_PENDING':
      return Object.assign({}, state, {
       isLoading: true,
       LoadingMsg: 'Please wait while fetching the Metrics data.'
    });

    case 'FETCH_COMPARE_SOLUTION_SUCCESS':
       console.log(action);
       var actvSession = state.activeSession;
       actvSession.CompareSolution = action.payload;
       return {
         ...state,
           activeSession: actvSession,
           isLoading: false,
